...
class HkPanedWindow:
    def __init__(self, master):
        ...
        self.panedwindow = ttk.Panedwindow(master, orient = HORIZONTAL, width = 900)
...
##        self.notebook.add(self.frame1, text = 'Non FX')
        ...
        def booking_confirmation():
            ...
            if not self.entry1.get().strip() or not self.entry2.get().strip() or not self.spinbox0.get().strip():
        ...
        def widgets(template):
            ...
            self.year = StringVar()
            self.currency1 = StringVar()
            self.currency2 = StringVar()
            ...
            self.label0 = ttk.Label(frame2, text = template, style = 'TLabel', font = ('Helvetica', 10, 'bold'))
            self.label0.grid(row = 0, column = 2, pady = 5)

            self.label1 = ttk.Label(frame2, text = 'Executing Entity', style = 'TLabel')
            self.label1.grid(row = 1, column = 0, padx = 15, pady = 5)

            self.combobox0 = ttk.Combobox(frame2, textvariable = self.executing_entity)
            self.combobox0.grid(row = 1, column = 1, pady = 5)
            self.combobox0.config(values = ('LOYD', 'LBCM', 'BOS', 'LBCW'))

            self.label2 = ttk.Label(frame2, text = 'Counterparty', style = 'TLabel')
            self.label2.grid(row = 1, column = 3, padx = 15, pady = 5)

            self.combobox1 = ttk.Combobox(frame2, textvariable = self.counterparty)
            self.combobox1.grid(row = 1, column = 4, pady = 5)
            self.combobox1.config(values = ('HSBC', 'Deutche', 'JaguarLandrover',
                                            'Morgan Stanely', 'JP Morgan', 'Citibank'))

            self.label3 = ttk.Label(frame2, text = 'Currency 1', style = 'TLabel')
            self.label3.grid(row = 2, column = 0, padx = 15, pady = 5)

            self.combobox2 = ttk.Combobox(frame2, textvariable = self.currency1)
            self.combobox2.grid(row = 2, column = 1, pady = 5)
            self.combobox2.config(values = ('GBP', 'EUR', 'USD','CAD',
                                            'JPY', 'CHF', 'AUD', 'NZD'))

            if template in ('Bond','Repo'):
                self.label4 = ttk.Label(frame2, text = 'Security', style = 'TLabel')
                self.label4.grid(row = 2, column = 3, padx = 15, pady = 5)

                self.entry0 = ttk.Entry(frame2)
                self.entry0.grid(row = 2, column = 4, pady = 5)

            if template not in ('Bond', 'Repo'):
                self.label5 = ttk.Label(frame2, text = 'Currency 2', style = 'TLabel')
                self.label5.grid(row = 2, column = 3, padx = 15, pady = 5)

                self.combobox3 = ttk.Combobox(frame2, textvariable = self.currency2)
                self.combobox3.grid(row = 2, column = 4, pady = 5)
                self.combobox3.config(values = ('GBP', 'EUR', 'USD','CAD',
                                                'JPY', 'CHF', 'AUD', 'NZD'))

            self.label6 = ttk.Label(frame2, text = '* Notional', style = 'TLabel')
            self.label6.grid(row = 3, column = 0, padx = 15, pady = 5)

            self.spinbox0 = ttk.Spinbox(frame2, textvariable = self.year)
            self.spinbox0.config(values = ('1M', '2M', '3M', '4M', '5M',
                                           '6M', '7M', '8M', '9M', '10M'))
            self.spinbox0.grid(row = 3, column = 1, padx = 15, pady = 5)

            self.label7 = ttk.Label(frame2, text = '* Start Date', style = 'TLabel')
            self.label7.grid(row = 4, column = 0, padx = 15, pady = 5)

            self.entry1 = ttk.Entry(frame2)
            self.entry1.grid(row = 4, column = 1, pady = 5)

            self.label8 = ttk.Label(frame2, text = '* End Date', style = 'TLabel')
            self.label8.grid(row = 4, column = 3, padx = 15, pady = 5)

            self.entry2 = ttk.Entry(frame2)
            self.entry2.grid(row = 4, column = 4, pady = 5)
            
            self.button0 = ttk.Button(frame2, text = 'Book', command = booking_confirmation)
            self.button0.grid(row = 5, column = 2, pady = 5)
